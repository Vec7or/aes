...
private:
    ...
    static constexpr std::array<std::byte, 10> roundConstant{std::byte{0x01}, std::byte{0x02}, std::byte{0x04}, std::byte{0x08}, std::byte{0x10}, std::byte{0x20}, std::byte{0x40}, std::byte{0x80}, std::byte{0x1B}, std::byte{0x36}};

    static inline constexpr void subkeyGFunction(unsigned int round, std::array<std::byte, 4> &input)
    {
    ...
    static inline constexpr std::array<std::byte, static_cast<size_t>(KEY_SIZE)> getSubkey(unsigned int round, std::array<std::byte, static_cast<size_t>(KEY_SIZE)> lastSubkey)
    {
        ...
        std::copy(std::begin(slices.back()), std::end(slices.back()), std::begin(gFunctionWord));
